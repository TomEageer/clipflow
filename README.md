...
- 尚无实际的剪贴板捕获 —— M1 才接 `NSPasteboard` 轮询。
...
| M1 | 剪贴板捕获：轮询 + 全 representation + concealed 过滤 | 下一步 |
...
| `docs/02-Paste底层实现分析.md` | 竞品底层逆向（只读分析，不含任何用户数据） |
